...
for batch_size in mini_batch_sizes:
    ...
    mlp.train(X=X_train, Y=Y_train, epochs=epochs, eta=best_eta, batch_size=256, momentum=True)
...
mlp = MLP(input_size=2, hidden_layer_sizes=best_layers)
loss_hist = mlp.train(X=X_train, Y=Y_train, epochs=epochs, eta=best_eta, batch_size=best_batch_size, momentum=True)
